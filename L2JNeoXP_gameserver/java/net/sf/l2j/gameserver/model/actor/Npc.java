...
public class Npc extends Creature
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.equalsIgnoreCase("TerritoryStatus"))
		{
			...
			if (clan != null)
			{
				...
				html.replace("%taxPercent%", getCastle().getCurrentTaxPercent());
			}
			else
				html.setFile(player.getLocale(), "html/territorynoclan.htm");
			
			html.replace("%territory%", (getCastle().getId() > 6) ? NpcStringId.ID_1001100.getMessage() : NpcStringId.ID_1001000.getMessage());
			html.replace("%townName%", getCastle().getTownName());
	...
	private static void showQuestWindowChoose(Player player, Npc npc, List<Quest> quests)
	{
		...
		for (final Quest q : quests)
		{
			...
			if (qs != null && qs.isStarted())
				sb.append(player.getSysString(10_075) + "<br>");
			else if (qs != null && qs.isCompleted())
				sb.append(player.getSysString(10_076) + "<br>");
			else
				sb.append("]</a><br>");
		}
	...
	public void showLotoWindow(Player player, int val)
	{
		...
		if (val == 0) // 0 - first buy lottery ticket window
			html.setFile(player.getLocale(), getHtmlPath(player, npcId, 1));
		else if (val >= 1 && val <= 21) // 1-20 - buttons, 21 - second buy lottery ticket window
		{
			...
			html.setFile(player.getLocale(), getHtmlPath(player, npcId, 5));
