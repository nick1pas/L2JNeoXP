...
public class DesireQueue
{
	...
	{
		final List<Desire> desires = _desires.stream().filter(d -> d.equals(desire)).collect(Collectors.toList());
		if (!desires.isEmpty())
	...
	public Desire getLast()
	{
		...
		return _desires.stream().max(Comparator.comparingDouble(d -> d.getWeight())).orElse(null);
	}
}