...
public class ClanHallManager implements IXmlReader
{
	...
	public void parseDocument(Document doc, Path path)
	{
		forEach(doc, "list", listNode -> forEach(listNode, "clanHall", chNode ->
		{
			...
			forEach(chNode, "buffs", buffsNode -> forEach(buffsNode, "buff", buffNode ->
			{
				int buffId = parseInteger(buffNode.getAttributes(), "id");
				int buffLvl = parseInteger(buffNode.getAttributes(), "lvl", 1);
				String buffDesc = parseString(buffNode.getAttributes(), "desc");
				int buffPrice = parseInteger(buffNode.getAttributes(), "price", 0);
				
				ch.addBuff(buffId, buffLvl, buffDesc, buffPrice);
			}));
