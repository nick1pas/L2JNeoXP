		...
		for (Entry<String, List<WalkerLocation>> route : list)
		{
			...
			else
			{
				...
				else
				{
					...
					list.append("<td width=150><a action=\"bypass admin_teleport ", teleLoc, "\">", StringUtil.trimAndDress(npc.getTemplate().getAlias(), 25), "</a></td><td width=40>", npc.getAI().getIndex(), " / ", route.getValue().size(), "</td><td width=50 align=right>", ((npc.isReversePath()) ? "Reverse" : "Regular"), "</td><td width=40 align=right><a action=\"bypass admin_show walker ", page, " ", route.getKey(), "\">Show</a></td>");
				}
			}
