...
public class Squash extends Events
{
	...
	private void addDrop()
	{
		...
		EventsDropManager.getInstance().addSquashRule(getName(), RuleType.ALL_NPC, event.itemId(), event.itemCount(), event.itemChance());
	}
		...
		for (int[] drop : DROPLIST)
		{
			if (npcId == drop[0])
			{
				if (chance < drop[1])
				{
					...
					((Monster) mob).dropItem(new IntIntHolder(itemId, itemQty));
					continue;
				}
			}
