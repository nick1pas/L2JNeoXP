...
public class WarriorUseSkill extends WarriorBehavior
{
	...
	public void onAttacked(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		...
		IntStream.range(1, 7).forEach(i ->
		{
			...
			{
				if (skillTarget == 0 && topDesireTarget != null && topDesireTarget instanceof Playable)
	...
	private static void warriorUseSkill(Npc npc, Creature creature, L2Skill skill, int skillProbability, int skillType, int desire, int checkDist, int distMin, int distMax, int hpTarget, int highHP, int lowHP, int mainAttack)
	{
		...
		if (Rnd.get(10000) < skillProbability)
		{
			...
			if (desire <= 0.0)
				return;
			else if ((hpTarget == 0 && ((npc.getStatus().getHpRatio() * 100) > highHP || (npc.getStatus().getHpRatio() * 100) < lowHP)) || (hpTarget == 1 && ((npc.getStatus().getHpRatio() * 100) > highHP || (npc.getStatus().getHpRatio() * 100) < lowHP)))
				return;
			else if (checkDist == 1 && (npc.distance2D(creature) < distMin || npc.distance2D(creature) >= distMax))
				return;
			else if (((checkDist == 1 && creature == npc && creature != null && (skillType == 0 || skillType == 2)) && creature != npc) && (npc.distance2D(creature) < distMin || npc.distance2D(creature) >= distMax))
				return;
